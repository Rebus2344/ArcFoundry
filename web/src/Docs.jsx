...
import './docs.css';
import {ContractDirectory} from './ContractDirectory.jsx';

const aliases={artwork:'factories',epochs:'prices',rent:'rewards',admin:'overview'};
const topicGroups=[
  ['Start here',['overview','mining','start']],
  ['Your Foundry',['factories','rarity','burn']],
  ['Money & token',['prices','rewards','trading','buybacks']],
  ['Reference',['contracts','privacy']]
];
function Contents({slug,copy}){
...
function FirstSteps(){return <section className="docs-first-steps" aria-labelledby="docs-first-steps-title">
...
export function Docs({config,rewardModel,slug='overview'}){
  const copy=newCopy;
  slug=Object.hasOwn(aliases,slug)?aliases[slug]:slug;
  const index=sections.indexOf(slug),section=index>=0?copy[slug]:null,heading=useRef(null),mobileMenu=useRef(null);
  useEffect(()=>{document.title=(section?.[0]||'Topic not found')+' · ArcFoundry Docs';if(mobileMenu.current)mobileMenu.current.open=false;heading.current?.focus({preventScroll:true});window.scrollTo({top:0,behavior:'auto'});},[slug]);
  return <div className="docs-layout">
    ...
    <article className="document docs-content" aria-labelledby="docs-title">
      ...
      {section?<>
        ...
        {slug==='prices'&&<div className="docs-table-scroll" tabIndex={0} role="region" aria-label="Epoch mint prices"><table><caption>Mint prices in USDC · issuance continues beyond this table</caption><thead><tr><th scope="col">Epoch</th><th scope="col">NFT numbers</th><th scope="col">USDC / mint</th></tr></thead><tbody>{Array.from({length:11},(_,e)=><tr key={e}><th scope="row">{e}</th><td>{String(epochStart(e))}–{String(epochStart(e)+epochSize(e)-1n)}</td><td>{eth(priceOf(epochStart(e)))}</td></tr>)}</tbody></table></div>}
        ...
        {slug==='contracts'&&<ContractDirectory config={config}/>}
